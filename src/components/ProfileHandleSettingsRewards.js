...
export default function ProfileHandleSettingsRewards() {
  // 2. Criar estado para o texto traduzido
  const [rewardsText, setRewardsText] = useState("Rewards");

  // useEffect para buscar a tradução
  useEffect(() => {
    const fetchTranslation = async () => {
      try {
        const result = await translate("Rewards", "en");
        setRewardsText(result);
      } catch (error) {
        console.error("Falha ao traduzir 'Rewards':", error);
      }
    };
      ...
      <View style={stylesProfile.container}>
        <View style={stylesProfile.boxProfile}>
          <View style={stylesProfile.rowContainer}>
            <Image
              source={require("../../assets/profile/RewardsIcon.png")}
